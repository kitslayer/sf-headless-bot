...
echo "ALERTS: trDead=$([ "${tr:-0}" -eq 0 ]&&echo YES||echo no) FROZEN2=$FROZEN2 bridges=$([ ${up:-0} -lt 4 ]&&echo DEGRADED||echo ok) supDead=$([ "${sup:-0}" -eq 0 ]&&echo YES||echo no) selfdestruct=$SD eval_due=$EVD"
